...
完善任务清单，支持多条待办整组勾选、批量调整完成状态并显示进度，修改后正文与渲染视图必须一致。空清单、未知标记、连续点击、嵌套列表、撤销以及重开文稿时，未知完成状态不能串行或丢失；清单变化还要同步字数、保存提示并保持其它 Markdown 内容不变。

### AI 回复

#### 现状分析

1. 复选框不可点击（`ignoreEvent() { return false }`，点击仅移动光标）
2. 解析器只识别 `[x]/[X]/[ ]`，未知标记（`[-]`、`[?]`）完全不识别；不支持空任务 `- [ ]`、有序任务 `1. [ ]`
3. 无批量操作、无进度显示
4. 初始加载即误标 dirty，保存提示不准确

#### 设计决策

1. **状态唯一来源是文本**：所有任务状态都从 Markdown 文本实时推导，不缓存——撤销、重开文稿后行为天然一致，正文与渲染视图永不脱节
2. **单字符替换不变量**：所有切换都是 ` ` ↔ `x` 的单字符替换，文档长度与位置永不偏移，连续点击互不干扰
3. **派发前重新校验**：点击切换时基于当前 state 重新读取该位置字符并确认是复选框，即使 Widget 持有过期位置也不会把错误字符写到错误位置（防串行）
4. **未知标记只读**：`[-]`、`[?]` 等识别为任务项并原样渲染（虚线框 + 原字符），点击与批量操作均不改写，保证不丢失
5. **批量 = 一次 dispatch**：整组勾选合并为单个事务，即一个撤销单元；幂等（已处于目标状态的项自动跳过）

#### 修改文件清单

- `src/editor/markdown-parser.js` — 任务正则支持任意单字符标记、有序列表、空任务
- `src/editor/task-list.js` — **新增**：任务收集/进度/单点切换/批量设置纯函数
- `src/editor/decoration-plugin.js` — CheckboxWidget 可点击切换；未知标记特殊渲染；已完成文本划线置灰
- `src/editor/index.js` — 导出新模块
- `src/stores/editor.js` — 任务进度统计；`updateContent` 支持 `markDirty` 选项
- `src/components/StatusBar.vue` — 显示 `✓ done/total` 进度
- `src/components/Toolbar.vue` — 新增任务组：插入任务、勾选全部、取消全部
- `src/App.vue` — 批量动作处理 + Ctrl/Cmd+S 保存
- `src/components/EditorPane.vue` — 初始加载不误标 dirty
- `src/styles/editor-theme.scss` — 复选框交互态、未知标记、已完成文本样式
- `src/editor/setup.js` — 默认文档加入嵌套/未知标记示例
- `README.md` — 新增任务清单说明章节

#### 生成的核心代码

##### `src/editor/task-list.js`（新增）
...
##### `src/editor/markdown-parser.js`（任务清单解析）
...
##### `src/editor/decoration-plugin.js`（CheckboxWidget 与装饰）
...
// buildDecorations 中的 task-list 分支：
case 'task-list': {
...
- 单元测试（解析器 + 任务操作纯函数）：**38 项全部通过**
  - 空清单/空文档、空任务 `- [ ]`、有序任务 `1. [ ]`、嵌套任务
  - 未知标记 `[?]` `[-]` `[~]` 识别为 known=false 且批量/点击均不改写
  - 连续点击幂等往返、过期位置校验拦截、选区范围批量、其它语法回归
- 端到端测试（jsdom 挂载真实 CM6 + 装饰插件）：**16 项全部通过**
  - 点击切换后正文与渲染视图一致、连续点击、未知标记只读
  - 撤销恢复、批量操作为单撤销单元、空任务/空文档不报错
- `npm run build` ✅ 构建成功
